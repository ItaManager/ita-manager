...
import {
  Search, Bell, Settings, Moon, Sun,
  LayoutDashboard, Users, Calendar, Clock,
  DollarSign, TrendingUp, Briefcase, Package,
  FileText, CheckSquare, UserPlus
} from "lucide-react";
...
export default function TestLayoutPage() {
  ...
  const totalEmployeesData = [
    { name: "Permanents", value: 42, color: "#10B981" },
    { name: "Contractuels", value: 18, color: "#F59E0B" },
    { name: "Journaliers", value: 85, color: "#3B82F6" },
  ];
  ...
  const menuItems = [
    ...
    { icon: Users, label: "Employés", active: false },
    { icon: CheckSquare, label: "Checklist", active: false },
    { icon: Calendar, label: "Congés", active: false },
    { icon: Clock, label: "Présences", active: false },
    { icon: DollarSign, label: "Paie", active: false },
    { icon: TrendingUp, label: "Performance", active: false },
    { icon: UserPlus, label: "Recrutement", active: false },
    { icon: Briefcase, label: "Projets", active: false },
    { icon: Package, label: "Ressources", active: false },
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 bg-card border-r border-border flex flex-col">
        ...
        <div className="p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-lg">ITA Manager</span>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border space-y-1">
          <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors cursor-pointer">
            <Settings className="size-5" />
            <span>Paramètres</span>
          </button>
          <button
            onClick={() => setDarkMode(!darkMode)}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors cursor-pointer"
          >
        ...
        <header className="sticky top-0 z-50 bg-card border-b border-border">
          <div className="flex items-center justify-between px-6 h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Avatar */}
              <button className="flex items-center gap-2 hover:bg-muted rounded-lg p-2 transition-colors cursor-pointer">
                <Avatar className="size-8">
                  <div className="w-full h-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-sm font-semibold text-white">
                    AG
                  </div>
                </Avatar>
              </button>
                  ...
                  {employees.map((emp) => (
                    <tr key={emp.id} className="border-b border-border hover:bg-muted/50 transition-colors cursor-pointer">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-sm font-medium text-emerald-600">
                      ...
                      <td className="py-3 px-4">
                        <Badge
                          ...
                          className={emp.statut === "Actif" ? "bg-emerald-500/10 text-emerald-700 hover:bg-emerald-500/20" : ""}
                        >
                          {emp.statut}
                        </Badge>
                      </td>
