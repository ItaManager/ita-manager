...
type Props = {
  ...
  demande: DemandeItem;
  onDecision: (decision: "VALIDER" | "REFUSER", motif?: string) => Promise<void>;
};
...
export function ModalDecision({
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      await onDecision(decision, motif.trim() || undefined);
      // Reset
      setDecision(null);
      setMotif("");
    } catch (error) {
            ...
            <div className="space-y-3">
              <div className="rounded-md border border-success-border bg-success-soft p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  La demande sera transmise à la Direction RH pour contrôle du
                  solde. Le collaborateur sera notifié de votre validation.
                </p>
              </div>
          ...
          {decision === "REFUSER" && (
            <div className="space-y-3">
              <div className="rounded-md border border-destructive bg-destructive/10 p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Le refus du supérieur hiérarchique est définitif. La demande
                  ne sera pas transmise à la RH.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="motif-refus"
                  placeholder="Expliquez la raison du refus au collaborateur..."
                  rows={4}
                  value={motif}
                  onChange={(e) => setMotif(e.target.value)}
                  required
