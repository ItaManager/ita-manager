...
/**
 * Upload d'un document employé
 * L'upload vers Supabase Storage doit être fait côté client,
 * cette action enregistre seulement les métadonnées en base
 */
export const enregistrerDocumentEmploye = actionProtegee(
  "employe:modifier",
  async (
    session,
    data: {
    ...
    const { employeId, typeDocument, nomFichier, cheminStorage, taille, estMedical } = data;
    ...
    await prisma.journalEvenement.create({
      data: {
        ...
        auteurId: session.userId,
        auteurNom: session.email,
        details: { typeDocument, nomFichier, employeId },
        commentaire: `Document uploadé pour ${employe.nom} ${employe.prenom}`,
      },
    });
...
/**
 * Supprimer un document employé
 * ATTENTION : ne supprime que l'enregistrement en base,
 * le fichier dans Supabase Storage doit être supprimé séparément
 */
export const supprimerDocumentEmploye = actionProtegee(
  "employe:modifier",
  async (session, documentId: string) => {
    ...
    await prisma.documentEmploye.delete({
      where: { id: documentId },
    });
    ...
    await prisma.journalEvenement.create({
      data: {
        ...
        commentaire: `Document supprimé pour ${document.employe.nom} ${document.employe.prenom}`,
      },
    });

    return { success: true, cheminStorage: document.cheminStorage };
  }
);
